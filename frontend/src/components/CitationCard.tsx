    ...
    >
      ...
      <div className="flex items-start gap-2.5 px-3.5 py-3">
        <div className={`flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center mt-0.5
          ${isExpanded ? 'bg-primary-200' : 'bg-slate-200'}`}>
          <FileText className={`w-3 h-3 ${isExpanded ? 'text-primary-700' : 'text-slate-500'}`} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <p className={`font-semibold leading-tight truncate ${isExpanded ? 'text-primary-800' : 'text-slate-700'}`}>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-white/70 rounded-md
                             border border-slate-200/80 text-slate-500">
