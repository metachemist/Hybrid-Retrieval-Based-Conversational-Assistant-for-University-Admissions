...
export default function DocumentsPage() {
  ...
  const inputClass = `w-full border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in-up stagger-1">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">PDF files are automatically chunked and indexed</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-1">
              ...
              <input
                ...
                accept=".pdf"
                required
                className="w-full text-sm text-slate-600 cursor-pointer
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in-up stagger-2">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">{docs.length} document{docs.length !== 1 ? 's' : ''} indexed</p>
          </div>
          <button
            onClick={fetchDocs}
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-slate-400">Upload an admission policy PDF to get started.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/60 border-b border-slate-100">
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500">Title</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-slate-500">Year</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-slate-500">Chunks</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-slate-500">Ingested</th>
